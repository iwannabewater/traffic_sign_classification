...
def predict(onnx_session, image):
    inputs = {onnx_session.get_inputs()[0].name: image.numpy()}
...
def main(image_paths):
    ...
    # load csv file to get GT
    labels_csv = "../csv_files/test.csv"
    ...
    for image_path in image_paths:
        ...
        pred_onnx = predict(onnx_session, image)
        y_pred_tags = np.argmax(pred_onnx, axis=2)[0]
        pred_class = labels[y_pred_tags[0]]
